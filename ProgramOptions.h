...
public:
    ...
    // shortcut for parsing option value as a list of strings, separated by 'separator'
...
// optional option with default value. if no default value is specified, the default value will
// be initialized using the default contructor of the given type.
template<typename T>
inline ProgramOption def(const char * name, const char * description, const T & default_value = T())
{
    return std::make_tuple(name, description, boost::program_options::value<T>()->
...
// be initialized using the default contructor of the given type.
template<typename T>
inline ProgramOption implicit(const char * name, const char * description, const T & default_value = T())
{
    return std::make_tuple(name, description, boost::program_options::value<T>()->
...
// bool specialization, no arguments allowed, set to true if specified w.o argument
template<>
inline ProgramOption def(const char * name, const char * description, const bool & default_value)
{
    return std::make_tuple(name, description, boost::program_options::value<bool>()->
                           default_value(default_value)->implicit_value(true)->zero_tokens());
}
...
{
    return std::make_tuple(name, description, boost::program_options::value<T>()->multitoken());
}
...
/**
 ...
 Code:
     lc::opt::choice<std::string>("tool", "required option", {"hammer", "saw"}) or
     lc::opt::choice<std::string>("tool", "required option", {"hammer", "saw"}, lc::opt::def::Required) */
