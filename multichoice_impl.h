...
public:
    invalid_choice_value(const std::string & bad_value)
        : boost::program_options::validation_error(boost::program_options::validation_error::invalid_option_value)
    {
...
{
public:
    multichoice_value(const std::vector<T> & choices = {}) : boost::program_options::typed_value<T>(0)
    {
        for (auto c : choices)
        {
    ...
    virtual void xparse(boost::any& value_store, const std::vector<std::string>& new_tokens) const
    {

        if (std::find(_choices.begin(), _choices.end(), new_tokens[0]) == _choices.end())
        {
